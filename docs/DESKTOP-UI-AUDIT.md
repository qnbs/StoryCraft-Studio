...
| C-5 | Medium | Component | Touch-target chunkiness under a mouse (`min-h-[44px]`/`min-w-[44px]` in desktop-visible toolbars; ~22 source files) | inline Tailwind across toolbars | pointer-aware `min-h-touch` utility (D3) | 🟡 D3 — `min-h-touch`/`min-w-touch` utilities (preset) + `--size-touch-target` token (44px → 36px under `.is-desktop` + `(pointer: fine)`, still WCAG AA). Adopted by the **Button** atom (cascades app-wide) + MindMapToolbar + ToolsPanel. Remaining scattered raw `min-h-[44px]` (Sidebar bottom-tab is touch — keep) migrate per the visual pass |

...
export const useCharacterGraphView = () => {
  ...
  const onAddRelationship = useCallback((fromId: string, toId: string, type: string, strength: number) => {
  ...
  const onUpdateRelationship = useCallback((relationshipId: string, updates: Partial<CharacterRelationship>) => {
  ...
  const onConnect = useCallback((fromId: string, toId: string) => {

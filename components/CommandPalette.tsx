...
import { useNavigate } from 'react-router-dom'; // Assuming routing, but adapting to current view state prop
  ...
  const commands: CommandItem[] = useMemo(() => {
    ...
    navItems.forEach(item => {
      cmds.push({
        id: `nav-${item.view}`,
        title: t(item.label),
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">{ICONS[item.icon]}</svg>,
    ...
    cmds.push({
      ...
      icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">{ICONS.ADD}</svg>,
    ...
    characters.forEach(char => {
      cmds.push({
        id: `char-${char.id}`,
        title: char.name,
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">{ICONS.CHARACTERS}</svg>,
    ...
    worlds.forEach(world => {
      cmds.push({
        id: `world-${world.id}`,
        title: world.name,
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">{ICONS.WORLD}</svg>,
    ...
    cmds.push({
      id: 'set-theme',
      title: settings.theme === 'dark' ? t('palette.action.lightMode') : t('palette.action.darkMode'),
      icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" /></svg>,
      category: t('palette.category.settings'),
      action: () => dispatch(settingsActions.setTheme(settings.theme === 'dark' ? 'light' : 'dark'))
    });

    cmds.push({
      id: 'set-lang',
      title: language === 'en' ? t('palette.action.switchLanguageToDe') : t('palette.action.switchLanguageToEn'),
      icon: <span className="font-bold text-xs border border-current rounded px-1">{language === 'en' ? 'DE' : 'EN'}</span>,
      category: t('palette.category.settings'),
      action: () => setLanguage(language === 'en' ? 'de' : 'en')
    });

    return cmds;
  }, [t, onNavigate, dispatch, settings.theme, language, characters, worlds]);
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] px-4">
      ...
      <div className="relative w-full max-w-2xl bg-[var(--background-secondary)]/90 backdrop-blur-xl border border-[var(--border-primary)] shadow-2xl rounded-xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200 ring-1 ring-white/10">
        ...
        <div className="flex items-center px-4 py-4 border-b border-[var(--border-primary)]/50">
          <svg className="w-5 h-5 text-[var(--foreground-muted)] mr-3" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`p-1.5 rounded-md ${isActive ? 'text-white bg-white/20' : 'text-[var(--foreground-secondary)] bg-[var(--background-tertiary)] group-hover:bg-[var(--background-primary)]'}`}>
                          {cmd.icon}
                        </div>
                        <span className={`font-medium ${isActive ? 'text-white' : ''}`}>{cmd.title}</span>
                      </div>
                      {cmd.shortcut && (
                        <div className="flex gap-1">
                          {cmd.shortcut.map(k => (
                            <kbd key={k} className={`px-1.5 py-0.5 text-xs rounded border ${isActive ? 'border-white/30 bg-white/20 text-white' : 'border-[var(--border-primary)] bg-[var(--background-primary)] text-[var(--foreground-muted)]'}`}>
                              {k}
                            </kbd>
                          ))}
                        </div>
                      )}
                      {isActive && <svg className="w-4 h-4 text-white opacity-70" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" /></svg>}
                    </button>
                  );
                })}
              </React.Fragment>
        ...
        <div className="hidden sm:flex items-center justify-between px-4 py-2 border-t border-[var(--border-primary)] bg-[var(--background-tertiary)]/30 text-xs text-[var(--foreground-muted)]">
            <div className="flex gap-3">
                <span className="flex items-center gap-1"><kbd className="bg-[var(--background-primary)] px-1 rounded border border-[var(--border-primary)]">↑</kbd><kbd className="bg-[var(--background-primary)] px-1 rounded border border-[var(--border-primary)]">↓</kbd> {t('palette.footer.navigate')}</span>
                <span className="flex items-center gap-1"><kbd className="bg-[var(--background-primary)] px-1 rounded border border-[var(--border-primary)]">↵</kbd> {t('palette.footer.select')}</span>
            </div>
